...
This release contains the complete We-Rise checkout, recurring billing, payment-history, failed-payment grace, card-update and cancellation flow. Live payments must remain disabled until the Supabase migrations, PayFast merchant configuration and sandbox tests are complete.
...
- Members can update card details on PayFast without We-Rise ever receiving the card number.
- Members can cancel future monthly charges through PayFast. Paid access remains available until the recorded paid-through date.
- Failed recurring payments enter a configurable grace period instead of cutting access off without warning.
- Complete, cancelled, failed, refunded and reversed notification events are kept in a server-only audit trail; refund/reversal ledger corrections are idempotent.
...
3. `supabase/migrations/0009_profile_photos_support.sql` — if it has not already returned Success.
4. `supabase/migrations/0010_gemini_payfast_production.sql`.
...
## 7. Enable the approved live membership flow

After the sandbox sequence passes, enter the LIVE PayFast merchant ID, key and exact passphrase on Render and set:
...
Then use **Save, rebuild, and deploy** on Render. Keep `PAYFAST_MERCHANT_ID`, `PAYFAST_MERCHANT_KEY` and `PAYFAST_PASSPHRASE` on Render only. The PayFast notification URL remains:

```text
https://werise-api.onrender.com/api/payfast/itn
```

In the admin Membership screen, confirm **Membership checkouts enabled** is switched on. Leave BackMi gift checkouts off unless PayFast separately approved that specific third-party gift and holding/payout flow.
