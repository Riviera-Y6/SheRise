...
- a mandatory live registration selfie step for We-Rise members;
- camera-only capture (no gallery picker) and a permanent selfie that cannot be replaced after it is saved;
...
4. Allow camera permission and take a live selfie. Confirm there is no gallery/file-picker option.
5. Save the selfie, then reopen it from the member header and confirm it is view-only and cannot be replaced.
6. Confirm Community and Messages show the selfie while logged in.
7. Open We-Rise in a logged-out/private browser and confirm public Community posts do not reveal profile photos.
8. Open **Support** while logged out.
9. Send a test request with a screenshot.
10. Confirm a `SUPPORT-...` reference appears in We-Rise.
11. Confirm the email reaches `request4.support@gmail.com` and that replying addresses the member directly.
...
## 5. Kirsten registration/waitlist update

Before deploying the September registration/waitlist update, also run:

`supabase/migrations/0012_registration_selfie_waitlist_goals.sql`

This adds Province/State and City/Town to waitlist records, allows new waitlist entries without the old Reason for Request field, and adds a database trigger that prevents a stored registration selfie from being replaced or cleared.

The Render API also removes a matching waitlist entry automatically when that email address becomes a registered/authenticated We-Rise member.
